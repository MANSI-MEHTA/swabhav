...
public class AddProductAction extends ActionSupport implements
	...
	public String executePost() {
		...
		HashMap<LineItem, Product> hashproductsincart = new HashMap<LineItem, Product>();
		List<LineItem> listoflineitemfromsession = (List<LineItem>) sessionmap
				.get("listoflineitem");
		...
		if (listoflineitemfromsession == null) {
			...
			addtolineitem();
		} else if (!lineitemservice.checklineitemexist(
				listoflineitemfromsession, productbeam.getId())) {
	...
	private void addtolineitem() {
		...
		List<LineItem> updatedlistofLineItems = lineitemservice.addLineItem(
				new LineItem(lineitemservice.generateLineItemId(), productbeam
						.getQuantity(), productbeam.getQuantity()
						* productbeam.getCost(), productbeam.getId()),
				(List<LineItem>) sessionmap.get("listoflineitem"));

		sessionmap.put("listoflineitem", updatedlistofLineItems);
	}
